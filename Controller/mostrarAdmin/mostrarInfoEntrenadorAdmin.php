<?php
    function mostrarInfoEntrenadorAdmin(){

        // Traemos la informacion de los usuarios desde el modelo, a partir de la clase y la funcion
        $objConsultas = new Consultas();
        $result = $objConsultas->mostrarInfoEntrenadorAdmin();

        if (!isset($result)) {
            echo '<h2>NO HAY ENTRENADORES REGISTRADOS</h2>';
        }else {
            foreach ($result as $f) {
                echo '
                    <tr>
                        ...
                        <td>' . $f['rol_usuario'] . '</td>
                        <td>' . $f['clavemd'] . '</td>
                        <td><a href="modificarEntrenador.php?id_user=' . $f['id_usuario'] . '" class="btn btn-success"> Ver/Editar</a></td>
                        <td><a href="../../Controller/eliminarAdmin/eliminarEntrenadorAdmin.php?id_user=' . $f['id_usuario'] . '" class="btn btn-danger"> Eliminar <i class="icofont-ui-delete"></i></a></td>
                    </tr>
                ';
            }
        }
    ...
    function cargarEntrenador(){
        ...
        $objConsultas = new Consultas();
        $result = $objConsultas->buscarEntrenador($id_user);

        foreach ($result as $f) {
            echo '
                <form action="../../Controller/modificarAdmin/modificarEntrenadorAdmin.php" method="POST" enctype="multipart/form-data">
                    <div class="row">
