<?php
    function mostrarEstadisticasLibres(){

        // Traemos la informacion de los usuarios desde el modelo, a partir de la clase y la funcion
        $objConsultas = new Consultas();
        $result = $objConsultas->mostrarEstadisticasLibresAdmin();

        if (!isset($result)) {
            echo '<h2>No Hay Medidas Registradas en el Sistema</h2>';
        }else {
            foreach ($result as $f) {
                echo '
                    <tr>
                        ...
                        <td><a href="modificarEstadisticasLibres.php?id_user=' . $f['id_usuario'] . '" class="btn btn-success"> Ver/Editar</a></td>
                        <td><a href="../../Controller/eliminarAdmin/eliminarEstadisticasLibresAdmin.php?id_user=' . $f['id_usuario'] . '" class="btn btn-danger"> Eliminar <i class="icofont-ui-delete"></i></a></td>
                    </tr>
                ';
            }
        }
    ...
    function cargarEstadisticasLibres(){
        $id_user = $_GET['id_user'];

        $objConsultas = new Consultas();
        $result = $objConsultas->buscarEstadisticasLibres($id_user);

        foreach ($result as $f) {
            echo '
                <form action="../../Controller/modificarAdmin/modificarEstadisticasLibresAdmin.php" method="POST">
                    <div class="row">
