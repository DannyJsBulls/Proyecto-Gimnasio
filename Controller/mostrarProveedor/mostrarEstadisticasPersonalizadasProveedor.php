<?php
    function mostrarEstadisticasPersonalizadasProveedor($id_cliente){

        // Traemos la informacion de los usuarios desde el modelo, a partir de la clase y la funcion
        $objConsultas = new Consultas();
        $result = $objConsultas->mostrarEstadisticasPersonalizadasProveedor($id_cliente);

        if (!isset($result)) {
            echo '<h2>No Tienes Estadisticas de Actividades Personalizadas Registradas en el Sistema</h2>';
        }else {
            foreach ($result as $f) {
                echo '
                    <tr>
                        ...
                        <!--<td><a href="modificarInscripcionesPersoCliente.php?id_user=' . $f['id_cliente'] . '" class="btn btn-success"> Ver Detalle</a></td>-->
                    </tr>
                ';
            }
        }
    ...
    function cargarEstadisticasPersonalizadasProveedor($id_cliente){
        $id_user = $_GET['id_user'];

        $objConsultas = new Consultas();
        $result = $objConsultas->buscarEstadisticasPersonalizadasProveedor($id_cliente);

        foreach ($result as $f) {
            echo '
                <form action="../../Controller/modificarEstadisticasPersonalizadasCliente.php" method="POST">
                    <div class="row">
